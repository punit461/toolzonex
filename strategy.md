...
- **New tool batches (colleague's tool-idea list, cross-checked against unitconverters.net and calculator.net's sitemap for completeness), decided per-tool feasibility first, then built what's realistic client-side-only:**
  ...
  - **Not yet built from the agreed list:** PDF manipulation tools (merge/split/rotate/etc. via pdf-lib), Word/Excel/PPT/EPUB→PDF conversions, face shape detector (would reuse the project's existing MediaPipe/TensorFlow.js dependencies), height comparison tool.

## Next steps (not yet done)

- Finish the remaining agreed tool batches: PDF manipulation (pdf-lib), document-to-PDF conversions, face shape detector, height comparison tool.
- Scale the paycheck calculator from 23 states to the remaining ~27 — the engine and page-generation pattern are proven, it's per-state tax-data entry (double-check each state's figures, especially ones with local/county taxes layered on top, before publishing).
