...
function lowercaseKeywords(sql: string): string {
  return sql.replace(TOKEN_REGEX, (match, stringLiteral, word) => {
    ...
    if (word && SQL_KEYWORDS.has(word.toUpperCase())) return word.toLowerCase();
    return match;
  });
}
...
const SqlKeywordsLowercaseContent = () => {
  const [input, setInput] = useState('');

  const output = useMemo(() => (input.trim() ? lowercaseKeywords(input) : ''), [input]);
...
const SqlKeywordsLowercase = () => {
  const content = (
    <>
      <Typography variant="h2">How to Use SQL Keywords Lowercase</Typography>
      <Typography variant="body1">
        ...
        lowercase, matched as a whole word and case-insensitively — while leaving table names, column names,
      ...
      <Typography variant="body1">
        ...
        lowercased, identifiers are left as-is, and the quoted string <code>'ACTIVE'</code> is untouched.
      </Typography>

      <Typography variant="h2">Common Use Cases</Typography>
      <Box sx={{ typography: 'body1' }}>
        <ul>
          <li>Matching a lowercase-keyword SQL style guide adopted by your team.</li>
          <li>Cleaning up all-caps SQL copied from an old codebase or textbook example.</li>
          <li>Preparing a query for a style checker or linter that expects lowercase keywords.</li>
        </ul>
      </Box>
  ...
  return (
    <CalculatorShell url="/developer-tools/sql-keywords-lowercase" content={content}>
      <SqlKeywordsLowercaseContent />
      <Box sx={{ mt: 4 }}><AdSenseUnit /></Box>
    </CalculatorShell>
  );
};

export default SqlKeywordsLowercase;
